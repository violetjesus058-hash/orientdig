 ...
 * 2. Replace [Orientdig Spreadsheet](docs.google.com) -> [Orientdig Spreadsheet](https://usfanslinki.com/)
...
function processArticle(filePath) {
  ...
  // Step 2: Check for existing Orientdig Spreadsheet anchor (case-insensitive)
  const ksRegex = /\[(orientdig spreadsheet)\]\(([^)]+)\)/gi;
  ...
  if (!hasKsAnchor) {
    ...
    const summaryNoLink = summary.replace(
      'visit the [Orientdig Spreadsheet](https://usfanslinki.com/) homepage',
      'visit the Orientdig Spreadsheet homepage'
    );
    content = content.trimEnd() + '\n\n' + summaryNoLink + '\n';
  }
